    ...
    try {
      ...
      for (const item of items) {
        ...
        `, [
          saleId,
          item.productId,
          item.productId, // variantId same as productId for now
          item.name || '',
          item.barcode || '',
          qty,
          item.priceMinor || 0,
        ...
        `, [device.store_id, item.productId]);
        ...
        `, [qty, device.store_id, item.productId]);
    ...
    try {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        for (const item of billItems) {
          await db.query(`
            INSERT INTO public.sale_items (sale_id, product_id, variant_id, name, barcode, quantity, price_minor, discount_minor, line_total_minor, created_at)
            VALUES ($1, $2::uuid, $3, $4, $5, $6, $7, 0, $8, $9)
            ON CONFLICT DO NOTHING
          `, [saleId, item.prod.id, item.prod.id, item.prod.name, item.prod.barcode, item.qty, item.prod.sell_price, item.lineTotalMinor, createdAt]);
        }
      ...
      await db.query(`
        INSERT INTO public.bill_sequences (store_id, last_number, prefix, updated_at)
        VALUES ($1::uuid, 120, 'B', NOW())
        ON CONFLICT (store_id) DO UPDATE SET last_number = 120, updated_at = NOW()
      `, [storeId]);

      console.log(`[Demo Seed] Completed for store ${storeId}:`, seeded);

      res.writeHead(200, { "Content-Type": "application/json" });
      return res.end(JSON.stringify({ success: true, storeId: store.id, storeName: store.name, storeCode: store.code, seeded }));
...
server.listen(PORT, () => {
  console.log(`Enroll service v3 listening on port ${PORT}`);
});
