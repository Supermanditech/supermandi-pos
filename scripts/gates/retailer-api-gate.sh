#!/bin/bash
# GCP-STG-0721: Retailer backend URL gate
BASE_URL="${1:-https://staging.supermandi.tech/api/v1}"
FAIL=0

echo "=== RETAILER BACKEND URL GATE ==="
check() {
...
check POST "/retailer-admin/auth/login" '{"email":"test@test.com","password":"test"}'
check GET "/retailer-admin/products"
check GET "/public/whatsapp-cta-config"
check GET "/admin/health"

[ "$FAIL" -eq 0 ] && echo "=== RETAILER API GATE: PASS ===" || { echo "=== RETAILER API GATE: $FAIL FAILED ==="; exit 1; }
