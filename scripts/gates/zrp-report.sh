...
# --- Phase 3: Routing Enforcement (L category) ---
ZRP_ROUTING_PASS=${ZRP_ROUTING_PASS:-0}
ZRP_ROUTING_FAIL=${ZRP_ROUTING_FAIL:-0}
ZRP_ROUTING_WARN=${ZRP_ROUTING_WARN:-0}
ZRP_ROUTING_INFRA_PASS=${ZRP_ROUTING_INFRA_PASS:-0}
ZRP_ROUTING_INFRA_FAIL=${ZRP_ROUTING_INFRA_FAIL:-0}
ZRP_ROUTING_SMOKE_PASS=${ZRP_ROUTING_SMOKE_PASS:-0}
ZRP_ROUTING_SMOKE_FAIL=${ZRP_ROUTING_SMOKE_FAIL:-0}

TOTAL_PASS=$((ZRP_A_PASS + ZRP_SECURITY_PASS + ZRP_MIGRATION_PASS + ZRP_CONFIG_PASS + ZRP_BUNDLE_PASS + ZRP_DOCKER_PASS + ZRP_LICENSE_PASS + ZRP_DB_ADV_PASS + ZRP_AUTH_HARD_PASS + ZRP_SCALE_PASS + ZRP_OBS_PASS + ZRP_ARTIFACT_PASS + ZRP_HYGIENE_PASS + ZRP_POSTURE_PASS + ZRP_CHAOS_EXT_PASS + ZRP_BIZ_PASS + ZRP_ROUTING_PASS + ZRP_ROUTING_INFRA_PASS + ZRP_ROUTING_SMOKE_PASS))
TOTAL_FAIL=$((ZRP_A_FAIL + ZRP_SECURITY_FAIL + ZRP_MIGRATION_FAIL + ZRP_CONFIG_FAIL + ZRP_BUNDLE_FAIL + ZRP_DOCKER_FAIL + ZRP_LICENSE_FAIL + ZRP_DB_ADV_FAIL + ZRP_AUTH_HARD_FAIL + ZRP_SCALE_FAIL + ZRP_OBS_FAIL + ZRP_ARTIFACT_FAIL + ZRP_HYGIENE_FAIL + ZRP_POSTURE_FAIL + ZRP_CHAOS_EXT_FAIL + ZRP_BIZ_FAIL + ZRP_ROUTING_FAIL + ZRP_ROUTING_INFRA_FAIL + ZRP_ROUTING_SMOKE_FAIL))
TOTAL_WARN=$((ZRP_A_WARN + ZRP_LICENSE_WARN + ZRP_ROUTING_WARN))
...
printf "| L: Routing Spec (CI)            | %4d | %4d | %4d |\n" "$ZRP_ROUTING_PASS" "$ZRP_ROUTING_FAIL" "$ZRP_ROUTING_WARN"
printf "| L: Routing Infra (CD)           | %4d | %4d |    0 |\n" "$ZRP_ROUTING_INFRA_PASS" "$ZRP_ROUTING_INFRA_FAIL"
printf "| L: Routing Smoke (CD)           | %4d | %4d |    0 |\n" "$ZRP_ROUTING_SMOKE_PASS" "$ZRP_ROUTING_SMOKE_FAIL"
echo "+---------------------------------+------+------+------+"
...
if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
  {
    ...
    echo "| L: Routing Spec (CI) | $ZRP_ROUTING_PASS | $ZRP_ROUTING_FAIL | $ZRP_ROUTING_WARN |"
    echo "| L: Routing Infra (CD) | $ZRP_ROUTING_INFRA_PASS | $ZRP_ROUTING_INFRA_FAIL | 0 |"
    echo "| L: Routing Smoke (CD) | $ZRP_ROUTING_SMOKE_PASS | $ZRP_ROUTING_SMOKE_FAIL | 0 |"
    ...
    echo ""
    echo "*Generated by ZRP Parity Report v3.0 — A-L categories*"
  } >> "$GITHUB_STEP_SUMMARY"
  echo ""
