...
const server = app.listen(config.port, () => {
  logger.info(`SuperMandi Order Service v3.0.9 running on port ${config.port}`, { env: config.env });
});

// T1-003: Graceful shutdown for Cloud Run SIGTERM
process.on('SIGTERM', () => {
  logger.info('[order-service] SIGTERM received, shutting down gracefully');
  server.close(() => {
    logger.info('[order-service] Server closed');
    process.exit(0);
  });
