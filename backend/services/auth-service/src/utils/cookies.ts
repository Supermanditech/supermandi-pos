// AUTH-STORAGE-001: HttpOnly Cookie Utility
// Migrates token storage from client-side localStorage to server-set HttpOnly cookies.
// Dual-mode: cookies are set alongside JSON body response for backward compatibility
// (POS app uses Authorization header, web clients use cookies).
...
export const AUTH_INDICATOR_COOKIE = 'sm_auth'; // Non-HttpOnly, readable by JS
...
/**
 * Extract refresh token from cookie header (for the refresh endpoint).
 * Falls back to request body for backward compatibility with POS app.
 */
