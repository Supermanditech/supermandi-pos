...
adminStaffRouter.get(
  ...
  async (req, res) => {
    ...
    // STG-425 FIX: sales.store_id is UUID (migration 018), purchases.store_id is TEXT
    // (ensureSchema.ts). Cast purchases comparison to text; sales is UUID=UUID natively.
    let query = `
