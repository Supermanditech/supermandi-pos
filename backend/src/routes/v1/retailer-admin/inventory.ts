...
// SCALE-C2: Expiry alerts endpoint
...
// =============================================================================
// SCALE-C2: EXPIRY ALERTS
// =============================================================================

/**
 * GET /api/v1/retailer-admin/inventory/expiring
 * Returns products nearing expiry for the authenticated store.
 *
 * Query params:
 *   daysAhead: number (default 90, max 365)
 *
 * Response:
 * {
 *   "expiring": [ { id, productId, name, barcode, quantity, expiryDate,
 *                   daysRemaining, batchNumber, severity } ],
 *   "counts": { "expired": 1, "critical": 3, "warning": 12 }
 * }
 */
retailerAdminInventoryRouter.get("/inventory/expiring", async (req: Request, res: Response) => {
  const pool = getPool();
  if (!pool) return res.status(503).json({ error: "database unavailable" });
  ...
  // Parse and clamp daysAhead (default 90 for retailer view)
  const rawDaysAhead = parseInt(String(req.query['daysAhead'] ?? '90'), 10);
  const daysAhead = isNaN(rawDaysAhead) || rawDaysAhead < 0 ? 90
  ...
  try {
    ...
    log.error("[SCALE-C2] Retailer expiry alerts error:", err.message);
