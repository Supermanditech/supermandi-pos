...
router.post("/create", registrationRateLimiter, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // This prevents cross-tab contamination when user opens registration in multiple tabs.
    ...
    // Expire any OTHER drafts for this phone (keep only the most recent one)
...
router.post("/submit-kyc", registrationRateLimiter, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // STG-420 FIX: Validate UUID format before query to prevent raw PG 22P02 error leaking to client
