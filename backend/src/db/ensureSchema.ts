...
export async function ensureCoreSchema(): Promise<void> {
  ...
  await pool.query(`
    ...
    CREATE INDEX IF NOT EXISTS inventory_ledger_store_id_idx ON inventory_ledger (store_id);
    CREATE INDEX IF NOT EXISTS inventory_ledger_product_id_idx ON inventory_ledger (global_product_id);
    CREATE INDEX IF NOT EXISTS inventory_ledger_store_product_idx ON inventory_ledger (store_id, global_product_id);
