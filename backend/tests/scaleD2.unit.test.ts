...
describe('SCALE-D2: Connection pool config defaults', () => {
  ...
  it('should have default DB_POOL_MIN=5', () => {
    const min = parseInt(process.env.DB_POOL_MIN || '5', 10);
    expect(min).toBe(5);
  });

  it('should have default DB_POOL_MAX=25', () => {
    const max = parseInt(process.env.DB_POOL_MAX || '25', 10);
    expect(max).toBe(25);
  });
