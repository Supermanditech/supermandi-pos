/**
 * GCP-STG-0444: GRNScreenV3 uses safe area insets for paddingTop + paddingBottom
 *
 * BEHAVIORAL test — renders GRNScreenV3 with mocked react-native-safe-area-context
...
jest.mock("../../services/api/orderApi", () => ({
  ...
  getOrder: jest.fn().mockResolvedValue({ items: [] }),
}));
...
describe("GCP-STG-0444: GRNScreenV3 safe area insets (BEHAVIORAL)", () => {
  beforeEach(() => {
  ...
  test("renders without crashing (smoke test)", () => {
    const tree = create(React.createElement(GRNScreenV3, defaultProps));
