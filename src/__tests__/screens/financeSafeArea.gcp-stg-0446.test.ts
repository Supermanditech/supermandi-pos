/**
 * GCP-STG-0446: FinanceScreenV3 uses safe area insets for paddingTop
 *
 * BEHAVIORAL test — renders FinanceScreenV3 with mocked react-native-safe-area-context
...
jest.mock("../../services/api/creditApi", () => ({
  ...
  applyForCredit: jest.fn().mockResolvedValue({}),
}));
...
describe("GCP-STG-0446: FinanceScreenV3 safe area insets (BEHAVIORAL)", () => {
  beforeEach(() => {
  ...
  test("renders without crashing (smoke test)", () => {
    const tree = create(React.createElement(FinanceScreenV3, defaultProps));
