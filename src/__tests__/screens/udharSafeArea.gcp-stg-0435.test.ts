/**
 * GCP-STG-0435: UdharScreenV3 uses safe area insets for paddingTop/paddingBottom
 *
 * BEHAVIORAL test — renders UdharScreenV3 with mocked react-native-safe-area-context
 * and verifies:
...
jest.mock("../../stores/cartStore", () => ({
  useCartStore: Object.assign(
    ...
    { getState: () => ({ lockCart: jest.fn(), unlockCart: jest.fn(), discount: null, clearCart: jest.fn() }) },
  ),
}));

jest.mock("../../services/api/posApi", () => ({
  createSale: jest.fn().mockResolvedValue({ saleId: "sale-1", totals: { totalMinor: 1000 } }),
  recordDuePayment: jest.fn().mockResolvedValue({}),
}));

jest.mock("../../services/api/apiClient", () => ({
  apiClient: { get: jest.fn().mockResolvedValue({ customers: [], data: [] }) },
}));
...
describe("GCP-STG-0435: UdharScreenV3 safe area insets (BEHAVIORAL)", () => {
  beforeEach(() => {
  ...
  test("renders without crashing (smoke test)", () => {
    const tree = create(React.createElement(UdharScreenV3, defaultProps));
