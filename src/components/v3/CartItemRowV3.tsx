...
export default function CartItemRowV3({ item, onIncrement, onDecrement, onRemove }: CartItemRowV3Props) {
  const colors = useThemeColors();
  const styles = useMemo(() => createStyles(colors), [colors]);
  const emoji = getCategoryEmoji(item.name);
  const lineTotal = item.priceMinor * item.quantity;
  const caseSize = item.metadata?.caseSize as number | undefined;
  const unit = (item.metadata?.unit as string) ?? "pcs";
  const brand = item.metadata?.brand as string | undefined;
  const hsnCode = item.metadata?.hsnCode as string | undefined;
