  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.footer}>
        ...
        {/* V3-HARDEN-089: Double-submit guard on approve-all */}
        <Pressable style={[styles.approveAllBtn, approving && { opacity: 0.5 }]} disabled={approving} onPress={async () => {
          if (approving) return;
          const online = await isOnline();
          if (!online) { showToast("Approve requires connection"); return; }
          const sid = await getDeviceStoreId();
          ...
          } catch { showToast("Failed to approve all"); } finally { setApproving(false); }
        }}><Text style={styles.approveAllText}>{approving ? "Approving..." : `Approve All (₹${total})`}</Text></Pressable>
      </View>
    </View>
  );
}
