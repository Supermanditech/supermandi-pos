    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsRow}>
          ...
          {/* V3-FIX-083: Real profit from API or "N/A" when unavailable */}
          <View style={styles.stat}><Text style={styles.statLabel}>Profit</Text><Text style={[styles.statVal, { color: colors.success }]}>{(summary as any)?.profit != null ? `₹${Math.round((summary as any).profit).toLocaleString("en-IN")}` : "N/A"}</Text><Text style={styles.statSub}>{(summary as any)?.profit != null ? `${Math.round(((summary as any).profit / (salesTotal || 1)) * 100)}% margin` : "cost data pending"}</Text></View>
        </View>
        ...
        <View style={styles.actionRow}>
          ...
          {/* V3-FIX-083: PDF export intentionally disabled until native PDF module is integrated.
              Keeping the button visible but explaining the limitation clearly. */}
          <Pressable style={[styles.actionBtn, { opacity: 0.5 }]} onPress={() => {
            showToast("PDF export coming soon — use Print or WhatsApp Share");
          }} disabled><Text style={styles.actionText}>📄 PDF</Text></Pressable>
        </View>
      </ScrollView>
    </View>
  );
