    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.actionRow}>
          ...
          {/* V3-FIX-083: PDF export shares same text content as print — uses system share sheet */}
          <Pressable style={styles.actionBtn} onPress={async () => {
            const text = `SuperMandi ${activeTab} Report\nSales: ₹${Math.round(salesTotal).toLocaleString("en-IN")} (${billCount} bills)\nCash: ₹${Math.round(cashAmount).toLocaleString("en-IN")}\nUPI: ₹${Math.round(upiAmount).toLocaleString("en-IN")}\nUdhar: ₹${Math.round(dueAmount).toLocaleString("en-IN")}`;
            try { const { Share } = require("react-native"); await Share.share({ message: text, title: `${activeTab} Report` }); } catch { showToast("Share cancelled"); }
          }}><Text style={styles.actionText}>📄 Export</Text></Pressable>
        </View>
      </ScrollView>
    </View>
  );
