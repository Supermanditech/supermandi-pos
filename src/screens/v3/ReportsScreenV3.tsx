    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.actionRow}>
          ...
          {/* GCP-STG-0047: PDF/Share export via native Share API */}
          <Pressable style={styles.actionBtn} onPress={() => {
            const content = `SuperMandi ${activeTab} Report\n\nSales: ₹${Math.round(salesTotal).toLocaleString("en-IN")} (${billCount} bills)\nCash: ₹${Math.round(cashAmount).toLocaleString("en-IN")}\nUPI: ₹${Math.round(upiAmount).toLocaleString("en-IN")}\nUdhar: ₹${Math.round(dueAmount).toLocaleString("en-IN")}\n\nGenerated: ${new Date().toLocaleString("en-IN")}`;
            Share.share({ message: content, title: `${activeTab} Report` }).catch(() => showToast("Share failed"));
          }}><Text style={styles.actionText}>📄 PDF</Text></Pressable>
        </View>
      </ScrollView>
    </View>
  );
