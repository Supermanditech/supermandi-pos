                ...
                <tr key={s.id}>
                  ...
                  <td>
                    <div style={{ display: "flex", gap: 6 }}>
                      <button className={s.is_active ? "btnDanger btnSm" : "btnSuccess btnSm"} onClick={() => { if (s.is_active && !confirm(`Deactivate staff member "${s.name}"?`)) return; handleToggleStaffActive(s.id, s.is_active); }} disabled={staffActionLoading === s.id} style={{ fontSize: 11, padding: "2px 8px" }}>
