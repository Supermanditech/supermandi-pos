...
async function transcribeWithWhisper(audioBuffer, filename) {
  return new Promise((resolve, reject) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode === 200 && result.text) {
            ...
            reject(new Error(result.error?.message || 'Transcription failed'));
          }
        } catch (e) {
    ...
    req.on('error', (e) => {
      console.error('[Whisper] Request error:', e);
      reject(e);
    });
...
async function parseMultipartAudio(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('end', () => {
      ...
      // Find audio file content
      let start = buffer.indexOf(boundaryBuffer);
      if (start === -1) return reject(new Error('No boundary found'));

      // Look for Content-Disposition with filename
      const headerEnd = buffer.indexOf(Buffer.from('\r\n\r\n'), start);
      if (headerEnd === -1) return reject(new Error('No header end'));
      ...
      const contentEnd = nextBoundary > 0 ? nextBoundary - 2 : buffer.length; // -2 for \r\n

      const audioBuffer = buffer.slice(contentStart, contentEnd);
      console.log('[Multipart] Parsed audio file:', filename, audioBuffer.length, 'bytes');
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && (url.pathname.startsWith("/voice/interpret") || url.pathname.startsWith("/interpret"))) {
    ...
    try {
      ...
      const { audioBuffer, filename } = await parseMultipartAudio(req);
      ...
      let transcript;
      try {
        transcript = await transcribeWithWhisper(audioBuffer, filename);
      } catch (whisperError) {
        console.error("[voice] Whisper error:", whisperError.message);
        res.writeHead(500, { "Content-Type": "application/json" });
        return res.end(JSON.stringify({ success: false, error: "Speech recognition failed: " + whisperError.message }));
      }
      ...
      // Store request for execute step
      voiceRequests.set(requestId, {
        requestId,
        transcript,
        intent,
        createdAt: new Date(),
      ...
      for (const [id, r] of voiceRequests.entries()) {
        if (r.createdAt < fiveMinutesAgo) voiceRequests.delete(id);
      }
  ...
  if (req.method === "POST" && (url.pathname.startsWith("/voice/execute") || url.pathname.startsWith("/execute"))) {
    ...
    try {
      ...
      if (!voiceRequest) {
        res.writeHead(404, { "Content-Type": "application/json" });
        return res.end(JSON.stringify({ success: false, error: "Request not found or expired" }));
      }
