...
// GO-LIVE-LOGIN: Forgot password flow with phone OTP verification
...
export default function ForgotPasswordPage() {
  ...
  const handleCheckPhone = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(API_GATEWAY_BASE + '/api/v1/retailer-admin/auth/forgot-password/request', {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(data.error?.message || 'Failed to verify phone');
      }
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(API_GATEWAY_BASE + '/api/v1/retailer-admin/auth/forgot-password/reset', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          idToken,
      ...
      {!isFirebaseReady() && step === 'phone' && (
        <div style={{
          ...
          <p style={{ margin: '0.5rem 0 0', fontSize: '0.875rem' }}>
            Password reset requires phone verification which is currently unavailable.
          </p>
        </div>
      )}
