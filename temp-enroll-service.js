...
const server = http.createServer(async (req, res) => {
  ...
  // ============================================================================
  // VOICE-003: Voice Interpret Endpoint (Mock STT)
  // ============================================================================
  if (req.method === "POST" && url.startsWith("/voice/interpret")) {
    console.log("[voice] POST /voice/interpret");

    // Simple mock transcription (no OpenAI on VM)
    const mockResponses = [
      "2 kilo rice chahiye",
      "toor dal 1 kilo",
      "sugar 5 kg add karo",
    ...
    const transcript = mockResponses[randomIndex];
    ...
    for (const [id, req] of voiceRequests.entries()) {
      if (req.createdAt < fiveMinutesAgo) voiceRequests.delete(id);
    }
  ...
  // ============================================================================
  if (req.method === "POST" && url.startsWith("/voice/execute")) {
    console.log("[voice] POST /voice/execute");
